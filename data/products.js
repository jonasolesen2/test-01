...
object3.method.call('hello johnny')
*/
...
export function loadProductsFetch() {
  const promise = fetch('https://supersimplebackend.dev/products'   
    ...
    products = productsData.map((eachProduct) => {
      if (eachProduct.sizeChartLink) {
        return new Clothing(eachProduct);
      }
       else if(eachProduct.type === 'appliance') {
        return new Appliance(eachProduct)
      }
...
export function loadProducts(fun) {
  ...
  xhr.addEventListener('load', () => {
    products = JSON.parse(xhr.response).map((eachProduct) => {
  if (eachProduct.sizeChartLink) {
    return new Clothing(eachProduct);
   } else if(eachProduct.type === 'appliance') {
    return new Appliance(eachProduct)
